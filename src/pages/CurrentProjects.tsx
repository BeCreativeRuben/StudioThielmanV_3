...
export default function CurrentProjects() {
  ...
  const cp = messages.currentProjects
  const currentProjects = getCurrentProjects(locale)
  const finishedProjects = getFinishedProjects(locale)
    ...
    <motion.div>
              ...
              >
                <div className="p-6 border-b border-gray-200">
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${statusColors[project.status]}`}>
                      {cp.status[project.status]}
                    </span>
                  </div>

                  <div className="mt-4">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm text-text-secondary">{cp.progress}</span>
                ...
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      <div className="text-xs text-text-secondary uppercase tracking-wider mb-1">{cp.started}</div>
                      <div className="text-sm font-semibold text-primary">{project.startDate}</div>
                    </div>
                    {project.estimatedCompletion && (
                      <div>
                        <div className="text-xs text-text-secondary uppercase tracking-wider mb-1">{cp.estimatedCompletion}</div>
                  ...
                  <div className="mb-4">
                    ...
                    >
                      <motion.div className="text-xs text-text-secondary uppercase tracking-wider">
                        {cp.plannedFeatures} ({project.plannedFeatures.length})
                      </motion.div>
                      <svg
                        ...
                        <ul className="space-y-1">
                          ...
                          {project.plannedFeatures.length > 3 && (
                            <li className="text-sm text-text-secondary">
                              +{project.plannedFeatures.length - 3} {cp.moreFeatures}
                            </li>
                          )}
                        </ul>
                      )}
                  ...
                  {expandedProject === project.id && project.discoveredItems.length > 0 && (
                    ...
                    >
                      <div className="text-xs text-text-secondary uppercase tracking-wider mb-2">{cp.discoveredLearned}</div>
                      <ul className="space-y-1">
                  ...
                  {project.notes && (
                    <div className="mb-4 p-3 bg-primary/5 border border-primary/20 rounded-lg">
                      <p className="text-sm text-text-primary italic">&ldquo;{project.notes}&rdquo;</p>
                    </div>
                  )}
      ...
      <section className="py-20 bg-gray-50">
        <motion.div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold text-primary mb-4">{cp.finished.title}</h2>
            <p className="text-lg text-text-secondary max-w-2xl mx-auto">{cp.finished.subtitle}</p>
          </motion.div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              >
                ...
                <motion.div className="p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    ...
                    {project.estimatedCompletion && (
                      <div>
                        <div className="text-xs text-text-secondary uppercase tracking-wider mb-1">{cp.completed}</div>
                  ...
                  <div className="flex gap-2 pt-4 border-t border-gray-200">
                    {project.liveUrl && (
                      ...
                      >
                        ...
                        {project.liveUrlText || cp.visitWebsite}
                      </a>
                    )}
                  </div>
                </motion.div>
      ...
      <section className="py-20 bg-gray-900">
        <motion.div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div
            ...
            transition={{ duration: 0.6 }}
          >
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">{cp.cta.title}</h2>
            <p className="text-lg text-white/80 mb-8">{cp.cta.subtitle}</p>
            <LocalizedLink to="/contact#contact-form">
              <Button variant="cta" size="lg">{cp.cta.button}</Button>
            </LocalizedLink>
          </motion.div>
        </motion.div>
      </section>
